...
namespace vulkan::pe
{
  ...
  public:
    ...
    static std::unique_ptr< image > create(HMODULE module);
    ...
    /// <summary>
    /// Converts a relative virtual address to a file offset and reports whether the conversion succeeded.
    /// </summary>
    bool try_rva_to_offset(std::uint32_t rva, std::uint32_t &offset) const noexcept;
    ...
    /// <summary>
    /// Converts a file offset to a relative virtual address and reports whether the conversion succeeded.
    /// </summary>
    bool try_offset_to_rva(std::uint32_t offset, std::uint32_t &rva) const noexcept;
    ...
    {
      return _nt_headers ? (std::uintptr_t)_nt_headers->OptionalHeader.ImageBase : 0;
    }
